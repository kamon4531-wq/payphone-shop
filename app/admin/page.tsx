      ...
      <div className="border rounded p-2">
        ...
        {(file || current) && (
          <div className="relative inline-block mb-1">
            ...
            {current && !file && (
              <button type="button" onClick={onClear} className="absolute -top-1 -right-1 bg-red-500 text-white rounded-full w-5 h-5 text-xs">×</button>
            )}
          </div>
        )}
  ...
  return (
    <div>
      <div className="flex flex-wrap gap-2 mb-3">
        <button onClick={() => setEditing({ category: "case", price: 0, old_price: null, description: "", badge_text: "" })} className="bg-emerald-500 text-white px-4 py-2 rounded-lg font-semibold">+ เพิ่มสินค้า</button>
        <input value={q} onChange={e => setQ(e.target.value)} placeholder="ค้นหาชื่อสินค้า..." className="flex-1 min-w-[200px] border rounded-lg px-3 py-2 text-sm"/>
      </div>

      <div className="flex gap-2 overflow-x-auto pb-2 mb-3">
        {CATEGORIES.map(c => (
          <button key={c.id} onClick={() => setCat(c.id)} className={`whitespace-nowrap px-3 py-1.5 rounded-full text-xs border transition ${cat === c.id ? "bg-emerald-400 border-emerald-400 text-black font-semibold" : "bg-white border-gray-200 text-gray-700 hover:border-emerald-300"}`}>
      ...
      {editing && (
        <form onSubmit={save} className="bg-white p-4 rounded-xl shadow mb-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <input type="number" placeholder="ราคาเก่า (ไม่บังคับ)" value={editing.old_price ?? ""} onChange={e => setEditing({ ...editing, old_price: e.target.value ? Number(e.target.value) : null })} className="border p-2 rounded"/>
          </div>
          ...
          <textarea placeholder="รายละเอียดสินค้า" value={editing.description || ""} onChange={e => setEditing({ ...editing, description: e.target.value })} rows={5} className="w-full border p-2 rounded resize-y"/>
...
function OrdersTab() {
  ...
  function exportCSV() {
    ...
    const rows = filtered.map(o => [
      ...
      o.customer_name, o.phone, o.branch || "", o.province || "",
      (o.address || "").replace(/\n/g, " "),
    ...
    <div>
      <div className="flex flex-wrap justify-between items-center gap-2 mb-3">
        ...
        <div className="flex gap-2 items-center">
          ...
          <button onClick={exportCSV} className="bg-blue-500 hover:bg-blue-600 text-white text-sm px-4 py-2 rounded-lg font-semibold">Export CSV</button>
        </div>
      </div>
      <div className="grid gap-2">
        {filtered.map(o => (
          <div key={o.id} className="bg-white p-3 rounded-xl shadow">
            ...
            {o.transfer_time && <div className="text-xs text-gray-600 mt-1">เวลาโอน: {new Date(o.transfer_time).toLocaleString("th-TH")}</div>}
